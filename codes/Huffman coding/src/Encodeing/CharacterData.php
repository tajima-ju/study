...
class CharacterData
{
    ...
    public function __construct(int|string|null $symbol)
    {
        ...
        $this->Create_Character_Data($this->character_list);
    }
    ...
    public function Create_Character_Data(array $character_list): void //文字配列を受け取り、データを作成、配列を作成する
    {
        $count = []; //文字=>出現回数 a=>1など...
        $first_position = []; //文字=>出現位置 
        $unsorted_character_data = []; //未整列データを一時格納
        ...
        //             [character] => a
        //         )
        usort($unsorted_character_data, function ($a, $b) { //第一優先count降順、第二優先first_position昇順
            $tmp = $a['count'] <=> $b['count'];
            if ($tmp === 0) {
        ...
        $this->character_data = $unsorted_character_data;
    }

    public function get_count_data(): array //huffmantree_dataに a=>4のような配列を生成して渡す
    {
        return array_column($this->character_data, 'count', 'character');
    }
